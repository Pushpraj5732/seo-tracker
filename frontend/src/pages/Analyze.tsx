...
import { useApp } from "../context/AppContaext";
        ...
        try {
            ...
            pollRef.current=setInterval(async()=>{
                ...
                if(attempt>maxAttempt){
                    ...
                    setError("analysis take too much time")
                    setAnalyzing(false);
                    return;
                }
                //fetch status
                try {
                    ...
                    if(status==="completed"){
                        ...
                        if(pollRef.current)clearInterval(pollRef.current);
                        setError(analysis.errorMessage || "Analysis failed");
                        setAnalyzing(false);
                    }else{
                        //update progress if we add percentage later
                        if(attempt>5)setCurrentStep(2)
                        
                    }
                } catch (err) {
                    console.error("Poll error:",err);
                    if(pollRef.current)clearInterval(pollRef.current);
                    setError("Failed to poll for status")
                    setAnalyzing(false);
                }
            },3000)
            
        } catch (error:any) {
            setError(error.response?.data?.message||error.message||"failed to start the analysis")
            setAnalyzing(false);
        }
    };
