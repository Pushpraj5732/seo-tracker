...
import { useApp } from "../context/AppContaext.js";
...
export default function RankTracker() {
            ...
            const pollInterval = setInterval(async () => {
                try {
                    ...
                    if (check.data.tracking.status !== "checking") {
                        ...
                        setKeywords((prev) => prev.map((i) => (i._id === id ? check.data.tracking : i)));
                    }
                } catch (error: any) {
                    console.error(error);
                }
    ...
    const handleRefresh = async (id: string) => {
        setRefreshing(id);
        try {
            //update stauts to checking
            await api.post(`/api/rank/${id}/refresh`)
            setKeywords((prev)=>prev.map((i)=>i._id===id ? {...i,status : "checking"}: i))
    ...
    const handleDelete = async (id: string) => {
       ...
       try {
        ...
        setKeywords((prev)=>prev.filter((i)=>i._id !== id))
       } catch (error) {
    ...
    const handleToggle = async (id: string) => {
        ...
        try {
            ...
            if(res.data.success){
                setKeywords((prev)=>prev.map((i)=>i._id===id ? {...i,active:res.data.tracking.active}:i))
            }   
        } catch (error) {
            console.log(error);
        }
